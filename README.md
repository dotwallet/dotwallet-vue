...
<template>
  ...
  <dotwallet-pay
    ...
    custom-class="my-override-class"
    op-return="your message"
    :receive-address="`[{"address": "1MS3HE9M3oEqW81KXp8iK9nBMTGXekdxAP","amount": 622}]`"
    :fetch-headers="{ 'API-Key': 'secret' }" :fetch-options="{ credentials: 'same-origin' }"
    :log="true" custom-class="my-override-class" @succes="handleSuccess" @fail="handleFail" >
...
| op-return       | string  |    NO    | A message or some stringified data to add to OP_RETURN                                                                                                                                                                                                              |
